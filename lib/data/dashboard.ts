...
async function attachProfiles<T extends { user_id: string }>(
...
): Promise<(T & { profiles: Pick<Profile, typeof fields[number]> | null })[]> {
  ...
  return items.map((item) => ({
    ...item,
    profiles: (map.get(item.user_id) as Pick<Profile, typeof fields[number]>) ?? null,
  }));
}
...
export async function getDashboardData(teamId: string) {
  ...
  const [entriesRes, monthEntriesRes, summariesRes, membersRes, activityRes] = await Promise.all([
    supabase
      .from('lunch_entries')
      .select('*')
      .eq('team_id', teamId)
  ...
  const totalPending = summaries.reduce((s, r) => s + Number(r.pending_amount), 0);
...
export async function getLunchEntries(
  teamId: string,
  opts?: { search?: string; status?: string; from?: string; to?: string; page?: number; limit?: number }
) {
  const supabase = await createClient();
  ...
  return { entries: entries as LunchEntryWithProfile[], total: count ?? 0, error };
}

export async function getTeamData(teamId: string) {
  const supabase = await createClient();
  const [team, membersRes, invitesRes, legacyInvitesRes, activityRes] = await Promise.all([
  ...
  const actUserIds = activityRaw.map((a) => a.user_id).filter(Boolean) as string[];
...
export async function getPublicTeamById(teamId: string) {
  ...
  const pending = (summaries.data ?? []).reduce((s, r) => s + Number(r.pending_amount), 0);
